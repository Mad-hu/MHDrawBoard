...
export class DrowBoard {
  ...
  removeDrawBoard() {
    --this.boardAddNum;
    console.log("removeDrawBoard", this.boardAddNum);
    this.drawBoard.remove(this.currentDrawGraph);
  }
  addDrawBoard() {
    ++this.boardAddNum;
    console.log("addDrawBoard", this.boardAddNum);
    this.drawBoard.add(this.currentDrawGraph);
  }

  boardMouseUp(options: fabric.IEvent<MouseEvent>) {
    const { offsetX, offsetY } = options.e;
    if (Math.abs(this.mouseFrom.x - offsetX) < 1 && Math.abs(this.mouseFrom.y - offsetY) < 1) {
      this.removeDrawBoard();
      this.currentDrawGraph = null;
      this.drawing = false;
      return;
  ...
  undo() {
    if (this.drawBoard._objects.length > 0) {
      const obj = this.drawBoard._objects.pop();
      console.log("undo obje:", obj);
   ...
   * 设置删除标志
   *
   * @param {boolean} [flag] false 停止删除  true 选中删除
   * @memberof DrowBoard
   */
